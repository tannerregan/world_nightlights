...
def arrays_to_polarsdf(arrays,names,mask_a):
    mask_c=np.transpose(np.reshape(mask_a,(1,-1))) #reshape array from 2D to 1D column
    
    #NB:ubercode uniquely identifies each cell 
    df = pl.DataFrame({"ubercode": range(len(mask_c)), 'mask': mask_c})
    
    for i,a in enumerate(arrays):
        print('adding new column: '+names[i])
        c=np.transpose(np.reshape(a,(1,-1))) #reshape array from 2D to 1D column
        df = df.with_columns(pl.lit(c).alias(names[i]))
    
    df_unmasked = df.filter((pl.col('mask')==False)).drop(['mask']) #exclude gas and AOI masked areas  
    return df_unmasked
...
def open_etr_train_data(dmsp_f,viirs_f,aoi_gas,aoi_bff,aoi_rgn,jdir,year,prf):     
    ...
    viirs_df=arrays_to_polarsdf(viirs_arrays,viirs_names, mask_array)
    ...
    fltr_df=arrays_to_polarsdf(fltr_arrays,fltr_names, mask_array)
    ...
    dmsp_df=arrays_to_polarsdf(dmsp_arrays,dmsp_names, mask_array)
    ...
    mrgd_df = viirs_df.join(fltr_df, left_on="ubercode", right_on="ubercode")
    del viirs_df, fltr_df
    mrgd_df = mrgd_df.join(dmsp_df, left_on="ubercode", right_on="ubercode")
    del dmsp_df
