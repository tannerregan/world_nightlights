...
def main(dmsp_viirs_zip,jdir,aoi_bff,aoi_gas,cln_f,sim_cln_f,aoi_prf):
    with zipfile.ZipFile(dmsp_viirs_zip, 'r') as zip_ref:    
        ...
        for zf in zip_lst:
            ...
            dmsp_a,bff_a,gas_a=open_rasters(dmsp_f,aoi_bff,aoi_gas) #open the rasters on an AOI window, output arrays
            check_shape(dmsp_a,bff_a,gas_a,dmsp_f) #check all shapes are the same
            apply_masks(dmsp_a,bff_a,gas_a,sim_cln_f.format(y=year+'sim'),aoi_prf) #Mask with AOI land and gasmask, save to file

#SCRIPT------------------------------------------------------------------------
if __name__ == "__main__":
    # execute only if run as a script
    main(dmsp_viirs_zip,jdir,aoi_bff,aoi_gas,cln_f,sim_cln_f,aoi_prf)
